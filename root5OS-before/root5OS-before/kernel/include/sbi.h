...
#define SBI_PMU_HW_STALLED_CYCLES_FRONTEND8
#define SBI_PMU_HW_STALLED_CYCLES_BACKEND9
...
#define SBI_PMU_HW_CACHE_OP_MAX3
...
sbi_legacy_call(uint64 eid, uint64 arg0, uint64 arg1, uint64 arg2, uint64 arg3)
{
...
sbi_call(uint64 eid, uint64 fid, uint64 arg0, uint64 arg1, uint64 arg2, uint64 arg3, uint64 arg4, uint64 arg5)
{
  struct sbiret ret;
  register uint64 a0 asm("a0") = arg0;
  register uint64 a1 asm("a1") = arg1;
  ...
  register uint64 a7 asm("a7") = eid;
  register uint64 err asm("a0");
  register uint64 val asm("a1");
  asm volatile("ecall"
    : "+r" (err), "+r" (val)
...
static inline void
sbi_shutdown(int sysfail)
{
  sbi_call(SBI_EXT_SRST, SBI_EXT_SRST_RESET, SBI_SRST_RESET_TYPE_SHUTDOWN,
    sysfail ? SBI_SRST_RESET_REASON_SYSFAIL : SBI_SRST_RESET_REASON_NONE,
    0, 0, 0, 0
  );
}

static inline int
sbi_hart_get_status(uint64 hartid)
{
  struct sbiret ret;
  ret = sbi_call(SBI_EXT_HSM, SBI_EXT_HSM_HART_GET_STATUS, hartid,
    0, 0, 0, 0, 0);
  if(ret.error){
    return ret.error;
  }
...
static inline void
sbi_set_timer(uint64 value)
{
  sbi_call(SBI_EXT_TIME, SBI_EXT_TIME_SET_TIMER, value,
    0, 0, 0, 0, 0);
}
